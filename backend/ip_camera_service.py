"""
IP Camera Service for EPI Recognition System

Handles CRUD operations for IP cameras with manufacturer-specific RTSP URL generation.
This is separate from CameraService which handles fueling monitoring cameras.
"""
...
class IPCameraService:
    ...
    def create_camera(
        ...
        try:
            ...
            return IPCameraService._row_to_ip_camera_dict(row)
    ...
    def list_cameras_by_user(db, user_id: str) -> List[Dict]:
        ...
        try:
            ...
            cameras = []
            for row in rows:
                cameras.append(IPCameraService._row_to_ip_camera_dict(row))
    ...
    def update_camera(
        ...
        try:
            ...
            if not update_fields:
                return IPCameraService.get_camera_by_id(db, camera_id)
            ...
            logger.info(f"✅ Updated IP camera {camera_id}")
            return IPCameraService._row_to_ip_camera_dict(row)
    ...
    def delete_camera(db, camera_id: int) -> bool:
        ...
        try:
            ...
            if deleted:
                logger.info(f"✅ Deleted IP camera {camera_id}")
            else:
                logger.warning(f"⚠️ IP camera {camera_id} not found")
    ...
    def _row_to_ip_camera_dict(row) -> Dict:
        """
        Convert database row to IP camera dictionary with password masking.

        Args:
            row: Database row tuple

        Returns:
            Camera dictionary with password masked
        """
        return {
            'id': row[0],
            'user_id': row[1],
